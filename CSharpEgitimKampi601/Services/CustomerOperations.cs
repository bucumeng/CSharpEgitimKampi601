...
namespace CSharpEgitimKampi601.Services
{
    public class CustomerOperations
    {
        ...
        {
            var connection = new MongoDbConnection(); // Mongo Db ye bağlanma isteğinde bulunduk
            var customerCollection = connection.GetCustomersCollection(); // Tabloya bağlandık

            var document = new BsonDocument // Parametreleri gönderdik
            {
            ...
            customerCollection.InsertOne(document); //Ekleme işlemi yaptık
        ...
        public List<Customer> GetAllCustomer()
        {
            ...
            foreach (var c in customers)
            {
                customerList.Add(new Customer
                {
                    ...
                    CustomerSurname = c["CustomerName"].ToString()
                });
            }
            return customerList;
        }
        ...
        public void UpdateCustomer(Customer customer)
        {
            ...
            {
                throw new ArgumentException("Müşteri ID boş olamaz.");
            }

            if (!ObjectId.TryParse(customer.CustomerId, out ObjectId objectId))
            {
                throw new FormatException("Müşteri ID geçerli bir ObjectId formatında değil.");
            }
            var connection = new MongoDbConnection();
            var customerCollection = connection.GetCustomersCollection();
        ...
        public Customer GetCustomerById(string id)
        {
            ...
            return new Customer
            {
                ...
                CustomerId = id,
                CustomerName = result["Customer"].ToString(),
